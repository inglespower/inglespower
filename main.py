...
# Función para generar audio ElevenLabs (ULaw 8kHz compatible Telnyx)
def generar_audio_elevenlabs(texto):
    try:
        ...
        if res.status_code == 200:
            ...
            audio_path = f"/tmp/tts_{timestamp}.ulaw"
...
async def webhook(request: Request):
    try:
        ...
        if not raw:
            print("[ERROR WEBHOOK JSON] Body vacío")
        ...
        if event_type == "call.initiated":
            ...
            time.sleep(1)  # Esperar 1 seg para que la llamada esté estable

            # Reproducir bienvenida
            bienvenida = "¡Hola! Soy InglesPower, tu asistente para practicar inglés. Dime lo que quieras practicar."
