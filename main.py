...
def telnyx_command(endpoint, payload=None):
    ...
    try:
        ...
        print(f"[ERR TELNYX API] Error conexión: {e}")
        return None
...
# Subir audio a Supabase y verificar que esté disponible
def subir_audio_supabase(local_path):
    try:
        ...
        # Esperar hasta que la URL esté accesible
        for i in range(10):  # hasta 10 intentos
            r = requests.head(public_url)
            if r.status_code == 200:
                return public_url
            time.sleep(0.5)  # espera medio segundo
        print(f"[ERR SUPABASE] URL no disponible después de 5s: {public_url}")
        return None
    except Exception as e:
