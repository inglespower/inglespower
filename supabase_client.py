...
# LEER DIRECTAMENTE DE RENDER (Soluciona el error de la foto)
SUPABASE_URL = os.environ.get("SUPABASE_URL")
SUPABASE_KEY = os.environ.get("SUPABASE_KEY")

if not SUPABASE_URL or not SUPABASE_KEY:
    raise ValueError("Error: SUPABASE_URL o SUPABASE_KEY no configurados en Render.")

supabase = create_client(SUPABASE_URL, SUPABASE_KEY)

def get_minutes(phone):
    res = supabase.table("minutes").select("*").eq("phone_number", phone).execute()
    # Supabase devuelve una lista, necesitamos el primer elemento [0]
